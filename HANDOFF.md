...
>
> ⚠️ **交接首要提醒（务必先读）**：当前 Git 工作树是**脏的**。线上运行版本为最近一次提交 `c1b0ba9`（"修复跨月后历史记录被隐藏…"），**仍含有【每日待办】与【时间管理】两个栏目**；而本地工作区有一轮**未提交、未部署**的改动，会删除这两个栏目、把箱型图每组改成 7 个并加 Tukey 显著性字母、把记账支出/收入默认收起。也就是说：**你手机上正在用的"小树"是旧版**，本地待提交的代码是新版。接手者首先要决定这些未提交改动是否提交并部署。
...
> 导航顺序由 `components/layout/nav-config.ts` 的 `NAV_ITEMS` 固定，共 **11 个栏目**（注意：本地工作区已删除待办/时间管理，但线上版本仍有，见 §1 提醒）。
...
**已删除但数据仍在（见 §1 提醒与 §10）：** `todo`、`timeThread`、`timeCell`、`timeMerge` 四个表的数据仍存于 IndexedDB 且仍被备份，但查看/恢复 UI 已不存在。
...
- **当前运行版本**：线上为最近提交 `c1b0ba9`；本地工作区有未提交的 4 项改动尚未部署（见 §1 提醒）。即**手机现在跑的是旧版**。
...
9. **工作树为脏**：见 §1 提醒——本地有未提交的 4 项改动，线上仍是旧版；接手第一件事是确认这些改动是否提交部署。
...
7. **⚠️ 当前工作树有未提交改动**（删待办/时间管理、箱型图 7 项+Tukey、记账默认收起），线上仍是旧版——先决定要不要提交部署。
